...
function questionsFor(state: WizardState): Question[] {
  ...
  questions.push({ id: 'auth', kind: 'single', label: 'Authentication', options: [{ value: 'none', label: 'No authentication' }, { value: 'api-token', label: 'Fixed API token', description: 'One fixed token attached to every request, verified on the server without a login flow.' }, { value: 'supabase-auth', label: 'Supabase Auth' }, { value: 'authjs', label: 'Auth.js' }, { value: 'privy', label: 'Privy' }], value: state.draft.authService, apply: (current, value) => ({ ...current, draft: { ...current.draft, authService: String(value) as ArchitectureDraft['authService'], loginMethods: [], authDetails: undefined } }) });
  if (state.draft.authService !== 'none') {
    if (state.draft.authService !== 'api-token') {
    ...
    questions.push({ id: 'auth-details', kind: 'text', label: 'Authentication details (optional)', placeholder: 'Token storage, rotation, or other authentication requirements', optional: true, value: state.draft.authDetails ?? '', apply: (current, value) => ({ ...current, draft: { ...current.draft, authDetails: String(value).trim() || undefined } }) });
  }
